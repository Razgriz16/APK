...
#################################################
with open('config/config.yaml', 'r') as file: config = yaml.safe_load(file)
...
#####################################################
def pkg_exe_error_msg_cursor(procedure_name, package_name):
    try:
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear variables para los parámetros de salida
...
##########################################################
######   IN PAR, OUT P_ERROR, OUT P_MSG, OUT NUMBER  #####
##########################################################
def pkg_exe_par_error_msg_number(procedure_name, package_name, input_params):
    try:
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear variables para los parámetros de salida
                p_numerooper = cursor.var(cx_Oracle.NUMBER)
                p_error = cursor.var(cx_Oracle.NUMBER)
                p_msg_error = cursor.var(cx_Oracle.STRING)

                # Construir la llamada al procedimiento dinámicamente
                param_placeholders = ", ".join(f":{key}" for key in input_params.keys())
                sql_call = f"""
                    BEGIN
                        {package_name}.{procedure_name}(
                            {param_placeholders},
                ...
                # Agregar los parámetros de salida al diccionario de entrada
                input_params["P_NUMEROOPER"] = p_numerooper
                input_params["P_ERROR"] = p_error
                input_params["P_MSG_ERROR"] = p_msg_error

                # Ejecutar el procedimiento
                cursor.execute(sql_call, input_params)

                # Obtener los valores de los parámetros de salida
                numero_operaciones = p_numerooper.getvalue()
                error_code = p_error.getvalue()
                error_message = p_msg_error.getvalue()
                ...
                # Retornar el número de operaciones
                return {"numero_operaciones": numero_operaciones, "error_code": error_code, "error_message": error_message}, 200
...
######################################################
##########   IN PAR, OUT P_ERROR, OUT P_MSG,  ########
######################################################
def pkg_exe_par_error_msg(procedure_name,package_name,parms):
    try:
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear variables para los parámetros de salida
                ...
                sql_call = f"""
                    BEGIN
                        {package_name}.{procedure_name}(
                            {input_placeholders},
                            :{num_input_params + 1},  -- Placeholder para P_ERROR
                            :{num_input_params + 2}   -- Placeholder para P_MSG_ERROR
                        );
                    END;
                """

                # Agregar los parámetros de salida al diccionario de entrada
                procedure_params = list(params) + [p_error, p_msg_error]
                ...
                # Verificar si hubo un error
                if error_code != 0:
                    return {"error": error_message,"error_code":error_code}, 400  # Devuelve el mensaje de error y código 400

                # Retornar el número de operaciones
                return {"message": error_message, "error_code": error_code}, 200
