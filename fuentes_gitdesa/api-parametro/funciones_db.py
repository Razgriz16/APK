...
#########################################
##########   NO PARAMS, OUT CURSOR  #####
#########################################
def pkg_exe_no_params_cursor(procedure_name, package_name):
    try:
        # Obtener una conexión del pool
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear la llamada al procedimiento
                sql_call = f"BEGIN {package_name}.{procedure_name}(:out_cursor); END;"

                # Crear una variable para el cursor de salida
                out_cursor = cursor.var(cx_Oracle.CURSOR)

                # Ejecutar el procedimiento con el cursor de salida
                cursor.execute(sql_call, [out_cursor])
