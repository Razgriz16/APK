...
######   IN PAR, OUT STRING  #####
##################################
def pkg_exe_par_string(procedure_name, package_name, params):
    try:   
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear variable para el parámetro de salida (cadena de texto)
                out_string = cursor.var(cx_Oracle.STRING)
                ...
                params["P_OUTPUT"] = out_string

                # Ejecutar el procedimiento
                cursor.execute(sql_call, params)

                # Obtener el valor del parámetro de salida
                out_varchar = out_string.getvalue()
                if out_varchar is not None:
                    out_varchar = out_varchar.strip()

                return {"data": out_varchar},200
...
######   IN PAR, OUT NUMBER  #####
##################################
def pkg_exe_par_number(procedure_name, package_name, input_params):
    try:   
        with get_connection() as connection:
            with connection.cursor() as cursor:
                # Crear variable para el parámetro de salida (cadena de texto)
